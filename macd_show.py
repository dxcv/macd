...
class Show_Macd:
    def __init__(self, root):
        ...
        Button(frame, text="从文件导入分钟数据", command=lambda: self.openfile(), font=("Arial", 12), width=20, height=2).pack(
            side=TOP)

        Button(frame, text="从wind导入近期分钟数据", command=lambda: self.wind_data_min(), font=("Arial", 12), width=20,
               height=2).pack(
            side=TOP)

        Button(frame, text="从wind导入近期日线数据", command=lambda: self.wind_data_day(), font=("Arial", 12), width=20,
               height=2).pack(side = TOP)

        Button(frame, text="得到价格分段数据", command=lambda: self.execute(), font=("Arial", 12), width=20, height=2).pack(
            side=BOTTOM)

        Button(frame, text = "波段统计", command = lambda: self.tongji(),font=("Arial", 12), width=20, height=2).pack(
            side=BOTTOM)

        Button(frame, text="储存", command=lambda: self.save(), font=("Arial", 12), width=20, height=2).pack(
            side=BOTTOM)
        ...
        l = Label(frame, text="请在第一行输入万德指数代码，第二行输入窗口时间(天),第三行输入存储位置", bg="green",
                  font=("Arial", 12), width=70, height=2)
    ...
    def price_split(self ,ptype):
        ...
        self.t1.insert(END, "波段开始时间          波段结束时间          波段确认时间          波段涨跌\n")

        for i in range(boduan.shape[0]):
            temp = boduan.iloc[i]
            self.t1.insert(END, str(temp.start_date) + "   " + str(temp.end_date) + "   " + str(temp.comfirm_date) + "   " +temp.bd_type + "\n")
    ...
    def save(self):
        ...
        day_limit_time_len.to_excel(writer_day, sheet_name="day_limit_time_len")
        day_continue_ret.to_excel(writer_day, sheet_name="day_continue_ret")
        day_limit_continue_ret.to_excel(writer_day, sheet_name="day_limit_continue_ret")
        day_continue_time.to_excel(writer_day, sheet_name="day_continue_time")
        day_limit_continue_time.to_excel(writer_day, sheet_name="day_limit_continue_time")

        min_boduan.to_excel(writet_minute,sheet_name="min_boduan")
        min_ret_sta.to_excel(writet_minute,sheet_name="min_ret_sta")
        min_limit_ret_sta.to_excel(writet_minute, sheet_name="min_limit_ret_sta")
        min_time_len.to_excel(writet_minute, sheet_name="min_time_len")
        min_limit_time_len.to_excel(writet_minute, sheet_name="min_limit_time_len")
        min_continue_ret.to_excel(writet_minute, sheet_name="min_ret_sta")
        min_limit_continue_ret.to_excel(writet_minute, sheet_name="min_continue_ret")
